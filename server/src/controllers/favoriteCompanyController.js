...
exports.addFavoriteCompany = async (req, res) => {
  ...
  try {
    const favorite = await FavoriteCompanyService.addFavoriteCompany(
      userId,
      companyId
    );
...
exports.getAllFavoriteCompanies = async (req, res) => {
  ...
  try {
    const favorites = await FavoriteCompanyService.getAllFavoriteCompanies(
      userId
    );
...
exports.deleteFavoriteCompany = async (req, res) => {
  try {
    ...
    const userIdFromLocals = res.locals.user.id;
    if (userId === userIdFromLocals) {
      const isDeleted = await FavoriteCompanyService.deleteFavoriteCompany(
        userId,
        companyId
      );
      if (isDeleted) {
        return res
          .status(200)
          .json({ message: "Favorite company deleted successfully" });
      } else {
